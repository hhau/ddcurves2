...
model {
  real sd_prior_inflation_factor;
  sd_prior_inflation_factor = 1.2;
  
  for (tt in 1:n_times) {
    // vectorwise over individuals.
    densities[tt,] ~ normal(fitted_values[tt,], sigma_curve);
    // beta_raw[tt, ] ~ normal(0, 1);
    // 2018-04-06 we're going to fix the hyper parameters at their posterior modes
    beta_midpoint[tt, 1] ~ normal(77.2, sd_prior_inflation_factor * 26.1);
    beta_midpoint[tt, 2] ~ normal(149.6, sd_prior_inflation_factor * 12.65);
  }
  ...
  beta_zero ~ normal(1023.842, sd_prior_inflation_factor * 0.183);
  beta_one ~ normal(1.085, sd_prior_inflation_factor * 0.113);
  beta_three ~ normal(42.8, sd_prior_inflation_factor * 80.9);
  //beta_four ~ normal(mean_beta_four, sigma_beta[5]);
  beta_six ~ normal(47.7, sd_prior_inflation_factor * 6.65);
  
  
  // 2018-04-06 we're going to fix the hyper parameters at their posterior modes
  // for well behaved chains
  ...
  // 
  // 
  // mean_beta_midpoint[1] ~ normal(15, 7);
  // mean_beta_midpoint[2] ~ normal(15, 7);
  ...
  sigma_curve ~ normal(0.07, 7.77e-4) T[0,];
}
